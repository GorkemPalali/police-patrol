...
from app.services.utils import (
    ...
    get_kucukcekmece_boundary,
    get_kucukcekmece_bbox_from_polygon,
    get_station_boundary,
)
...
def get_station_coordinates_snapped(
    ...
    snapped = _snap_to_road_network(
        db, original_lat, original_lng, max_distance_m, station_id=str(station_id)
    )
...
def get_high_risk_road_segments(
    ...
    # Filter by station's boundary if available, otherwise neighborhoods, then Küçükçekmece boundary
    boundary_geom = None
    boundary_filter = ""
    ...
    if station_id:
        station = db.query(PoliceStation).filter(PoliceStation.id == station_id).first()
        if station and station.boundary_geom:
            boundary_filter = """
                -- Center point must be within station boundary
                ST_Within(
                    ST_Centroid(geom::geometry),
                    (SELECT boundary_geom::geometry
                     FROM police_station
                     WHERE id = CAST(:station_id AS uuid))
                )
                AND
            ...
            """
        elif station and station.neighborhoods:
            boundary_filter = """
                ...
                ST_Within(
                    ...
                    (SELECT ST_Union(ab.geom::geometry)::geometry
                     ...
                     WHERE ps.id = CAST(:station_id AS uuid)
...
def get_high_risk_cells(
    ...
    """Get risk cells above threshold within station boundaries or Küçükçekmece boundary and time window"""
    from app.services.utils import get_kucukcekmece_boundary, get_station_neighborhood_boundaries
    ...
    # Filter by station boundary if available, otherwise neighborhoods, then Küçükçekmece polygon
    boundary_geom = None
    boundary_filter = ""
    if station_id:
        station = db.query(PoliceStation).filter(PoliceStation.id == station_id).first()
        if station and station.boundary_geom:
            boundary_geom = station.boundary_geom
            boundary_filter = """
                ST_Within(
                    ST_Centroid(geom::geometry),
                    (SELECT boundary_geom::geometry
    ...
    if boundary_geom and boundary_filter:
        query = query.filter(text(boundary_filter).params(station_id=station_id))
    elif boundary_geom:
        # Use polygon intersection for Küçükçekmece
        query = query.filter(
            text("""
                ST_Intersects(
                    geom,
    ...
    return query.order_by(RiskCell.risk_score.desc()).limit(100).all()


def _get_station_boundary_bbox(
    db: Session,
    station_id: str
    ...
    result = db.execute(
        text("""
            SELECT
                ST_YMin(boundary_geom::geometry) as min_lat,
                ST_XMin(boundary_geom::geometry) as min_lng,
                ST_YMax(boundary_geom::geometry) as max_lat,
                ST_XMax(boundary_geom::geometry) as max_lng
            FROM police_station
            WHERE id = CAST(:station_id AS uuid)
    ...
    if not result or result.min_lat is None:
        result = db.execute(
            text("""
                WITH boundary AS (
                    SELECT ST_Union(ab.geom::geometry) as geom
                    FROM administrative_boundary ab
                    JOIN police_station ps ON ab.name = ANY(ps.neighborhoods)
                    WHERE ps.id = CAST(:station_id AS uuid)
                    AND ab.admin_level = 8
                ...
                SELECT
                    ST_YMin(boundary.geom) as min_lat,
                    ST_XMin(boundary.geom) as min_lng,
                    ST_YMax(boundary.geom) as max_lat,
                    ST_XMax(boundary.geom) as max_lng
                FROM boundary
...
def _ensure_forecast_heatmap(
    ...
    try:
        if time_window_start.tzinfo:
            start_naive = time_window_start.replace(tzinfo=None)
        else:
            start_naive = time_window_start
        if time_window_end.tzinfo:
            end_naive = time_window_end.replace(tzinfo=None)
        else:
            end_naive = time_window_end
        
        existing = db.execute(
            text("""
                SELECT COUNT(*) as count
                FROM risk_cell
                WHERE time_window && tsrange(:start_time, :end_time, '[]')
            """),
            {"start_time": start_naive, "end_time": end_naive},
        ).scalar()
        
        if existing and int(existing) > 0:
            return
    except Exception as exc:
        logger.warning(f"Failed to check existing heatmap cells: {str(exc)}")
        db.rollback()
    
    try:
        from app.api.routes.forecast import generate_forecast_risk_cells
        
        generate_forecast_risk_cells(
            db=db,
        ...
        logger.warning(f"Forecast heatmap generation failed: {str(exc)}")
        db.rollback()
...
def cluster_risk_cells(
    ...
    if clusters:
        ...
        snapped_coords = snap_risk_cell_centroids(
            db,
            cluster_coords,
...
def snap_risk_cell_centroids(
    ...
    for lat, lng in centroids:
        snapped = _snap_to_road_network(db, lat, lng, max_distance_m, station_id=station_id)
        if snapped:
...
def _snap_to_road_network(
    ...
    try:
        ...
        boundary_params = {}
        if station_id:
            station_boundary_exists = db.execute(
                text("""
                    SELECT boundary_geom IS NOT NULL
                    FROM police_station
                    WHERE id = CAST(:station_id AS uuid)
                """),
                {"station_id": station_id},
            ).scalar()
            if station_boundary_exists:
                boundary_filter = """
                    AND ST_Within(
                        v.the_geom::geometry,
                        (SELECT boundary_geom::geometry
                ...
                """
                boundary_params["station_id"] = station_id
            else:
...
def _get_boundary_filter_for_routing(
    db: Session,
    ...
    if station and station.neighborhoods:
        return """
            AND ST_Within(
                ST_Centroid(geom::geometry),
                (SELECT ST_Union(ab.geom::geometry)::geometry
...
def _compute_route_with_pgrouting(
    ...
    """
    Compute route between two vertices using pgRouting A* (fallback to Dijkstra).
    ...
    """
    try:
        boundary_filter = _get_boundary_filter_for_routing(db, station_id)

        base_query = """
            SELECT id, source, target, cost, reverse_cost
            FROM road_segment
            WHERE road_type IN ('motorway', 'trunk', 'primary', 'secondary', 'tertiary',
                                'residential', 'service', 'unclassified')
        """
        pgrouting_query = f"""{base_query} {boundary_filter}"""
        pgrouting_query_astar = f"""
            SELECT id, source, target, cost, reverse_cost,
                   ST_X(ST_StartPoint(COALESCE(geom_geom, geom::geometry))) as x1,
                   ST_Y(ST_StartPoint(COALESCE(geom_geom, geom::geometry))) as y1,
                   ST_X(ST_EndPoint(COALESCE(geom_geom, geom::geometry))) as x2,
                   ST_Y(ST_EndPoint(COALESCE(geom_geom, geom::geometry))) as y2
            FROM road_segment
            WHERE road_type IN ('motorway', 'trunk', 'primary', 'secondary', 'tertiary',
                                'residential', 'service', 'unclassified')
            {boundary_filter}
        ...
        try:
            ...
            logger.warning(f"pgRouting A* failed, falling back to Dijkstra: {str(exc)}")
...
def _compute_network_distance(
    ...
    try:
        ...
        logger.warning(f"pgRouting A* distance failed, falling back to Dijkstra: {str(exc)}")
    ...
    try:
        ...
        logger.warning(f"pgRouting Dijkstra distance failed: {str(exc)}")

    return None


def _tsp_route_length(order: List[int], dist_matrix: List[List[float]]) -> float:
    if not order:
        return 0.0
...
def _tsp_nearest_neighbor(dist_matrix: List[List[float]]) -> List[int]:
...
def _tsp_two_opt(order: List[int], dist_matrix: List[List[float]]) -> List[int]:
    if len(order) < 3:
        return order

    best = order[:]
    best_length = _tsp_route_length(best, dist_matrix)
    improved = True
    ...
    while improved:
        ...
        for i in range(len(best) - 1):
            for j in range(i + 1, len(best)):
                ...
                candidate_length = _tsp_route_length(candidate, dist_matrix)
...
def optimize_waypoints_tsp(
    db: Session,
    start_lat: float,
    start_lng: float,
    waypoints: List[Tuple[float, float]],
    waypoint_risk_scores: List[float],
    station_id: Optional[str] = None,
) -> Tuple[List[Tuple[float, float]], List[float]]:
    if len(waypoints) <= 1:
        return waypoints, waypoint_risk_scores

    points = [(start_lat, start_lng)] + waypoints
    snapped_vertices = []
    for lat, lng in points:
        snapped = _snap_to_road_network(db, lat, lng, max_distance_m=200.0, station_id=station_id)
        if snapped:
            snapped_vertices.append((snapped[0], snapped[1], snapped[2]))
        else:
            snapped_vertices.append((lat, lng, None))
    ...
    for i in range(len(points)):
        ...
        for j in range(len(points)):
            if i == j:
                row.append(0.0)
                continue
            v1 = snapped_vertices[i][2]
            v2 = snapped_vertices[j][2]
            if v1 is not None and v2 is not None:
                network_dist = _compute_network_distance(db, v1, v2, station_id=station_id)
                if network_dist is not None:
                    row.append(network_dist)
                    continue
            row.append(
                _haversine_distance(points[i][0], points[i][1], points[j][0], points[j][1])
            )
        dist_matrix.append(row)

    initial_order = _tsp_nearest_neighbor(dist_matrix)
    optimized_order = _tsp_two_opt(initial_order, dist_matrix)

    ordered_waypoints = [waypoints[i - 1] for i in optimized_order]
    ordered_risks = [waypoint_risk_scores[i - 1] for i in optimized_order]

    return ordered_waypoints, ordered_risks
...
def compute_route_via_points(
    ...
    try:
        ...
        if len(waypoints) == 0 and abs(start_lat - end_lat) < 0.0001 and abs(start_lng - end_lng) < 0.0001:
            ...
            vertex_boundary_filter = ""
            vertex_params = {"lat": start_lat, "lng": start_lng}
            if station_id:
                station_boundary_exists = db.execute(
                    text("""
                        SELECT boundary_geom IS NOT NULL
                ...
                if station_boundary_exists:
                    ...
                    """
                    vertex_params["station_id"] = station_id
                else:
                    station_neighborhoods = db.execute(
                        text("""
                            SELECT neighborhoods
        ...
        for lat, lng in all_points:
            snapped = _snap_to_road_network(
                db, lat, lng, max_distance_m=100.0, station_id=station_id
            )
            if snapped:
...
def compute_route(
    ...
    station_boundary = get_station_boundary(db, str(request.station_id))
    if station_boundary:
        # Station boundary will be used for filtering, no bbox needed
        bbox = None
    else:
    ...
    if request.start_time and request.end_time:
        try:
            ...
            forecast_bbox = _get_station_boundary_bbox(db, str(request.station_id))
            if not forecast_bbox:
                forecast_bbox = (
                    get_kucukcekmece_bbox_from_polygon(db)
                    or settings.kucukcekmece_fallback_bbox
                )
            
            _ensure_forecast_heatmap(db, start_dt, end_dt, forecast_bbox, use_hex=True)
            ...
            try:
                ...
                db.rollback()
                logger.warning(f"Failed to read forecast risk cells: {str(exc)}")
                risk_cells = []
            
            if risk_cells:
                risk_clusters = cluster_risk_cells(
                    db,
                    risk_cells,
                    max_clusters=10,
                    station_id=str(request.station_id),
                ...
                logger.info(
                    "Found %s forecast risk cells above threshold %s, clustered into %s",
                    len(risk_cells),
                    request.risk_threshold,
                    len(risk_clusters),
                )
                ...
                logger.info("Created %s waypoints from forecast heatmap", len(waypoints))
            else:
                logger.warning("No forecast risk cells found, falling back to road segments")
        except (ValueError, TypeError) as exc:
            logger.warning(f"Failed to parse forecast time window: {str(exc)}")
    ...
    if not waypoints:
        try:
            ...
            db.rollback()
            logger.warning(f"Failed to read high-risk road segments: {str(exc)}")
            high_risk_segments = []
        ...
        if len(waypoints) > 10:
            sorted_indices = sorted(
                range(len(waypoint_risk_scores)),
    ...
    if not waypoints:
        ...
        logger.info(
            "Found %s risk cells above threshold %s, clustered into %s",
            len(risk_cells),
            request.risk_threshold,
            len(risk_clusters),
        )
    ...
    if not waypoints:
        ...
        boundary_filter = _get_boundary_filter_for_routing(db, str(request.station_id))
    ...
    if waypoint_risk_scores:
        waypoint_risk_scores = waypoint_risk_scores[:len(waypoints)]

    if waypoints and waypoint_risk_scores:
        waypoints, waypoint_risk_scores = optimize_waypoints_tsp(
            db,
            start_lat,
            start_lng,
            waypoints,
