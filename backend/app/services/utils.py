...
def geojson_polygon_to_geography(geojson: dict) -> Geography:
    """Convert GeoJSON Polygon to PostGIS Geography."""
    if not geojson:
        raise ValueError("Boundary GeoJSON is empty")
    if geojson.get("type") != "Polygon":
        raise ValueError("Boundary geometry must be a GeoJSON Polygon")
...
def get_station_boundary(
    ...
    """
    Get the boundary polygon for a station.
    Prefers station boundary_geom if available, falls back to neighborhood boundaries.
    """
    ...
    if station.boundary_geom:
        return station.boundary_geom

    return get_station_neighborhood_boundaries(db, station_id)
...
def get_kucukcekmece_bbox_from_polygon(db: Session) -> Optional[Tuple[float, float, float, float]]:
    ...
    try:
        geom_param = boundary_geom.data if hasattr(boundary_geom, "data") else boundary_geom
        result = db.execute(
            text("""
                SELECT 
                    ST_YMin(ST_GeomFromWKB(:geom, 4326)) as min_lat,
                    ST_XMin(ST_GeomFromWKB(:geom, 4326)) as min_lng,
                    ST_YMax(ST_GeomFromWKB(:geom, 4326)) as max_lat,
                    ST_XMax(ST_GeomFromWKB(:geom, 4326)) as max_lng
            """),
            {"geom": geom_param},
        ).first()
