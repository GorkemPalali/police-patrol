...
from app.services.utils import (
    lat_lng_to_geography,
    get_point_coordinates,
    geography_to_geojson,
    geojson_polygon_to_geography,
...
def list_stations(
    ...
    for station in stations:
        ...
        boundary = geography_to_geojson(db, station.boundary_geom) if station.boundary_geom else None
        results.append(
            PoliceStationRead(
                id=station.id,
                name=station.name,
...
def create_station(payload: PoliceStationCreate, db: Session = Depends(get_db)):
    ...
    if payload.boundary:
        try:
            boundary_geom = geojson_polygon_to_geography(payload.boundary.model_dump())
...
def update_station(
    ...
    if payload.boundary:
        try:
            station.boundary_geom = geojson_polygon_to_geography(payload.boundary.model_dump())
        except ValueError as exc:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=str(exc),
