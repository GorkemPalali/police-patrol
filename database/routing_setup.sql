...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='road_segment' AND column_name='geom_geom') THEN
        ALTER TABLE road_segment ADD COLUMN geom_geom geometry(LINESTRING, 4326);
    END IF;
END $$;

-- Geometri uzunluğuna dayalı maliyet hesapla Maliyet = mesafe metre cinsinden
UPDATE road_segment
SET geom_geom = COALESCE(geom_geom, geom::geometry),
    cost = ST_Length(COALESCE(geom_geom, geom::geometry)),
    reverse_cost = CASE 
        WHEN one_way THEN 1e9  -- Very high cost for reverse direction on one-way roads
        ELSE ST_Length(COALESCE(geom_geom, geom::geometry))
    END
