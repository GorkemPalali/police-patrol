  ...
  const handleOptimizeRoute = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        ...
        setError(detail ? `Rota oluşturulurken hata oluştu: ${detail}` : 'Rota oluşturulurken hata oluştu');
        console.error('Route optimize failed:', err.response?.data || err);
      } else {
      ...
      >
        ...
        <div style={{ marginBottom: '24px', paddingBottom: '24px', borderBottom: '1px solid #333' }}>
          ...
          {!showAddCrimeForm ? (
            ...
            <div>
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '12px', color: '#cccccc', fontWeight: '500' }}>
                  Severity (Suç Seviyesi): <span style={{ color: '#0066ff', fontWeight: '600' }}>{crimeFormData.severity}</span>
                </label>
                <input
                  type="range"
                  min="1"
        ...
        <div style={{ marginBottom: '24px', paddingBottom: '24px', borderBottom: '1px solid #333' }}>
          ...
          <p style={{ margin: 0, marginBottom: '12px', fontSize: '11px', color: '#999' }}>
            OSM'den çekilen araç yollarını gösterir.
          </p>
          <button
            onClick={async () => {
              if (!showTrafficMap) {
          ...
          >
            {loadingTraffic ? 'Yükleniyor...' : (showTrafficMap ? 'Trafik Ağını Gizle' : 'Trafik Ağını Göster')}
          </button>
        ...
        <div style={{ marginBottom: '24px', paddingBottom: '24px', borderBottom: '1px solid #333' }}>
          ...
          <p style={{ margin: 0, marginBottom: '12px', fontSize: '11px', color: '#999' }}>
            Öngörülen risk yoğunluğunu gösterir.
          </p>
          <div style={{ marginBottom: '12px' }}>
            <label style={{ display: 'block', marginBottom: '6px', fontSize: '12px', color: '#cccccc', fontWeight: '500' }}>
              Başlangıç:
